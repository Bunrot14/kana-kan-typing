...
# -*- coding: utf-8 -*-
"""
generate_static_pages_like_index_v4.py

変更点：
- 「トップページへ戻る」ボタンを、問題文欄（#display）の**すぐ上・左寄せ**に配置（固定表示を廃止）
- Xでシェアボタンは前後ナビより**控えめ**（ghost/outline）に
- 前/次ボタンは**主役（pillグラデ）**のまま
- .criticism-link のスタイルを**指定通り**に適用（問題文の裏庭／お気に入りページ／note）
- 既存の index 風デザイン、5リンク、下部3リンク、お気に入り、WPM計測は維持
- OGP/Xカードは従来同様（固定値）
"""
import os
import re
...
# ===== サイト設定 =====
SITE_NAME    = "かな漢タイピング"
SITE_URL     = "https://www.typingwithclavier.com"
PROBLEMS_DIR = "problems"
OG_IMAGE_URL = f"{SITE_URL}/assets/OGP_Clavier.png"

# X(Twitter) カード
TWITTER_CARD_TYPE = "summary_large_image"
TWITTER_TITLE     = "かな漢タイピング"
TWITTER_DESC      = "文学的な日本語長文で心を整えるタイピング体験を。"
TWITTER_IMAGE     = OG_IMAGE_URL

def make_description(text: str, maxlen: int = 160) -> str:
    t = text.replace("\\n", " ").replace("\n", " ")
...
<body>
  ...
  <div class="row-top-back">
    <a class="btn back soft" href="{{ root }}/index.html" aria-label="トップページへ戻る">
  ...
  <!-- 本文ソース（SEOのため静的埋め込み） -->
  <pre id="sourceText" class="visually-hidden" style="display:none">{{ content }}</pre>
  ...
  <!-- 5リンク -->
  <div class="link-row">
    <a href="{{ root }}/clavier_collection.html" class="archive-link">クラヴィエの寓話集</a><span class="sep">｜</span>
    <a href="{{ root }}/archive.html" class="archive-link">すべてのバックナンバー</a><span class="sep">｜</span>
    <a href="{{ root }}/Criticism_index.html" class="criticism-link">問題文の裏庭（解説アーカイブ）</a><span class="sep">｜</span>
    <a href="{{ root }}/favorites.html" class="criticism-link">お気に入りページ</a><span class="sep">｜</span>
    <a href="https://note.com/kanakantyping" class="criticism-link">note</a>
  </div>

  <div class="pager">
  {% if prev_url %}
    <a class="btn pill nav" href="{{ prev_url }}" aria-label="前の問題">
  ...
  <a class="btn ghost share" href="https://x.com/intent/post?text={{ share_text_enc }}&url={{ url_enc }}" target="_blank" rel="noopener" aria-label="Xでシェア">
  ...
  {% if next_url %}
    <a class="btn pill nav" href="{{ next_url }}" aria-label="次の問題">
      <span>次の問題</span>
  ...
  <footer>
    <p style="margin: 0;">
      <a href="{{ root }}/meditation_typing.html" class="archive-link">瞑想的タイピングのすすめ</a> |
      <a href="{{ root }}/philosophy.html" class="archive-link">問題文に込めた想い</a> |
      <a href="{{ root }}/Misconversion.html" class="archive-link">誤変換のポエジー</a>
    </p>
  </footer>
...
def main():
    ...
    for entry in data:
        ...
        essays = entry.get("essays") or []
        if not date_str: 
            continue
        ymd = datetime.strptime(date_str, "%Y-%m-%d").strftime("%Y%m%d")
        for i, _essay in enumerate(essays, start=1):
            file_name = f"{ymd}_{i:02}.html"
            url_index[(date_str, i)] = f"{SITE_URL}/{PROBLEMS_DIR}/{file_name}"

    success, skip = 0, 0
    for entry in data:
        date_str = entry.get("date")
        essays = entry.get("essays") or []
        if not date_str or not essays:
            skip += 1
            continue

        ymd = datetime.strptime(date_str, "%Y-%m-%d").strftime("%Y%m%d")
        for i, essay in enumerate(essays, start=1):
            title = (essay.get("title") or "").strip() or f"{date_str}の問題 {i}"
            text  = (essay.get("text") or "").rstrip()
            tags  = essay.get("tags") or []
            if not text:
                skip += 1
                continue

            file_name = f"{ymd}_{i:02}.html"
            output_path = os.path.join(PROBLEMS_DIR, file_name)
            page_url = url_index[(date_str, i)]
            prev_url = url_index.get((date_str, i-1))
            next_url = url_index.get((date_str, i+1))

            description = make_description(text, 160)
            share_text  = f"{title}｜{SITE_NAME} #かな漢タイピング"
            share_text_enc = quote_plus(share_text)
            url_enc = quote_plus(page_url)
    ...
    print(f"✅ 生成完了: {success} 件 / スキップ: {skip} 件")
    print("ℹ️ sitemap は別スクリプトで生成してください。")
